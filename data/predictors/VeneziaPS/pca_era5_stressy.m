...
addpath('C:\Users\Rodrigo\Desktop\Rodrigo\02 - Research fellow\01 - Storm surge ML downscaling\Surge-repository\surge-ml\data\coarse data\era5')

% .nc file
filename = 'era5_windstress.nc';
...
par = ncread(filename,'stressY');

%% Load Med-MFC coordinates to interpolate ERA5

% Add Med-MFC coarse data path
addpath('C:\Users\Rodrigo\Desktop\Rodrigo\02 - Research fellow\01 - Storm surge ML downscaling\Surge-repository\surge-ml\data\coarse data\med-mfc')

% Med-MFC .nc file
filemed = 'medmfc_ssh.nc';

% Extract Med-MFC coordinates
latcmems = ncread(filemed,'lat');
loncmems = ncread(filemed,'lon');

%% ERA5 matrix interpolation (at every timestep) to CMEMS matrix dimensions

latcmems_sort = sort(latcmems,'descend');
loncmems_sort = sort(loncmems,'descend');   
[LATcmems_sort,LONcmems_sort] = meshgrid(latcmems_sort,loncmems_sort);

m = size(par);
for i = 1:m(3)
    s = par(:,:,i);
    msl_int(:,:,i) = interp2(LAT,LON,s,LATcmems_sort,LONcmems_sort);
end

%% Copy of original coordinates and variable of interest

LAT_filt = LATcmems_sort;
LON_filt = LONcmems_sort;
par_filt = msl_int;

%% Pre process of the data to use
...
pred_coord = double([45.4,12.63]);  % Punta della Salute (VeneziaPS)
...
save('stressy.txt','serie','-ascii'); % File to be saved on .txt format
